  ...
  useEffect(() => {
    async function fetchAdvice() {
      ...
      if (!resetRetry && currentRetryCount >= 5 && cache?.isError) {
        setAdvice("");
        setError(`Exceeded 5 attempts to get advice. Please try again tomorrow. (${currentRetryCount} attempts)`);
        setLoading(false);
        return;
      }
      ...
      try {
        ...
        // Handle error response from the API
          ...
          const errorMsg = data.error.message || "Error getting farming advice.";
          setError(errorMsg);
          ...
          if (newRetryCount >= 5) {
            setError(`Exceeded 5 attempts to get advice. Please try again tomorrow. (${newRetryCount} attempts)`);
          }
        }
