...
//
// WARNING:
//	Use caution when using 3v only processors like arm and ESP8266
//processors 	when interfacing with 5v modules as not doing proper level shifting
//or 	incorrectly hooking things up can damage the processor.
//
...
//	the hd44780 library can be installed using the Arduino IDE library
//manager 	starting with IDE 1.6.2 or can found here:
//		https://github.com/duinoWitchery/hd44780
//
...
//	WARNING: 3v only systems like ARM/Teensy3/ESP8266 devices will need to
//take 		precautions on SDA and SCL connections if using 5V I2C devices. 		Level
//shifters are recommended and should be used. 		It is possible to cheat and hook
//the pullups to 3v instead of 5v. 		If doing this, there can be no other pullups
//that are conected to 5V 		on any device that is connected to the bus. 		Many of
//the backpacks ind other devices nclude 5v pullups on them 		so if pullups to 3v
//are being used to cheat, the backpack or other 		device will likely require
//hardware modification. 		Failure to do this 3v pullup "cheat" correctly can
//damage the processor 		which is why using actual level shifters is recommended.
//
// 3. Connect to the board using the IDE serial monitor; set the baud rate to
// 9600
...
//
//	It is recommended to connect to the serial monitor *before* you upload
//the 	sketch so that the serial monitor is reconnected as soon as the sketch is
//	uploaded to avoid character loss.
//	This is particularly helpful on boards that use a USB virtual serial
//port 	like leonardo.
//
...
//		Scroll down to modify the defines: LCD_CHEXECTIME,
//LCD_INSEXECTIME
//
// -----------------------------------------------------------------------
// Expected behavior
...
//		NOTE: Arduino 2560 boards have 10k external pullups on the
//arduino board 		      Arduino DUE V3 boards have 1.5k external pullups on the arduino
//board
...
//		The ESP8266 core use GPIO bit numbers and a few boards use Dn or
//Pn defines 		to do do pin # mapping to bit numbers. 		The code attempts to
//accurately report that information.
...
// Uncomment and use this line instead of the one below if you have a SYDZ
// backpack
// hd44780_I2Cexp lcd[1]={{I2Cexp_ADDR_UNKNOWN, I2Cexp_BOARD_SYDZ}}; // to run
// on a single SYDZ based backpack
 ...
 * for for each and every use of F() and PSTR() macros on older versions of the
 * IDE.
 */
...
void setup() {
  ...
  for (int n = 0; n < NumLcd; n++) {
    ...
    lcd[n].setCursor(0, 1);
    for (int c = 0; c < LCD_COLS; c++) lcd[n].write(' ');
  }
...
// confusing since some variants (notably WeMos D1 and NodeMCU) decided to
// include Dn defines to do pin to bit mapping. This means that using pin N is
// the not same as using Dn with those variants. i.e. D5 may not be the same pin
// as 5
//
...
// This mismatched mapping is usually the case for Pn mapping variants for the
// i2c pins. Most variants use GPIO 4 for SDA and GPIO 5 for SCL and do not use
// or/ include Dn pin mapping defines. The Wemos D1 and NodeMCU board variants
// have really made a mess of things. Those variants include Dn pin mappings and
// D4 is sometimes 4 and D5 is not 5. And some boards have a D14 and D15 labels
...
void printDigitalPin(Print & outdev, int pin) {
  ...
  if (pin == D0) outdev.print(F(" D0"));
  if (pin == D1) outdev.print(F(" D1"));
  if (pin == D2) outdev.print(F(" D2"));
  if (pin == D3) outdev.print(F(" D3"));
  if (pin == D4) outdev.print(F(" D4"));
  if (pin == D5) outdev.print(F(" D5"));
  ...
  if (pin == P3) outdev.print(F(" P3"));
  if (pin == P4) outdev.print(F(" P4"));
  if (pin == P5) outdev.print(F(" P5"));
  if (pin == P6) outdev.print(F(" P6"));
  if (pin == P7) outdev.print(F(" P7"));
  if (pin == P8) outdev.print(F(" P8"));
...
int pullupOnPin(uint8_t pin) {
  ...
  // now the code will simply fail to compile ungracefully if the symbol doesn't
  // exist
